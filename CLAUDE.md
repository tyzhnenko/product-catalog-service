...
Filtering on these dynamic maps happens via query-string mini-DSLs parsed in `src/core/utils.py`, which build raw Mongo filter dicts. `build_attribute_filter` handles the repeated `attrs=key:value` param (e.g. `attrs=color:red&attrs=color:blue` → OR via `$in`; different keys → AND). `build_price_search_filter` handles the single `price` string param, shlex-split into whitespace-separated tokens: `<key>>=<value>` / `<key><=<value>` for the top-level price map, and `loc:<id>[:<key>[<op><value>]]` / `region:<code>[:<key>[<op><value>]]` for `location_price`/`region_price` (id alone or id+key with no operator means an existence check; `>=`/`<=` tokens on the same path merge into one range condition). Products have a wildcard Mongo index on `attributes.$**` to support this.
